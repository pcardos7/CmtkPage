...
const dataFields = ["Contact Temperature Contact Temperature [°C]", "Vibration Velocity RMS v-RMS X [mm/s]", "Vibration Velocity RMS v-RMS Y [mm/s]", "Vibration Velocity RMS v-RMS Z [mm/s]"];
...
// Interval ID for data fetching
let getDataIntervalID;
const SAMPLING_TIME = 10000; // 10 seconds
...
function ChartPage() {
		...
		async function fetchPorts() {
			...
			try {
				...
				for (let port of ports) {
					const response = await fetch(API_ENDPOINTS.getPortName(cmtk, port));
	...
	useEffect(() => {
		async function fetchCmtkData() {
			if (!cmtk || !location || !selectedField || !selectedTime || !selectedPort) return;

			setLoading(true);
			setError(null);
			try {
				// Makes the GET request to get the read data
				const response = await fetch(API_ENDPOINTS.getReadData(cmtk, selectedTime, selectedPort));
				...
				// Store data
				const selectedData = jsonData.data.map((item) => item[selectedField] ?? null);
		...
		getDataIntervalID = setInterval(fetchCmtkData, SAMPLING_TIME);

		return () => clearInterval(getDataIntervalID); // Clear interval on component unmount
	...
	useEffect(() => {
		async function GetWarningState() {
			try {
				...
				const response = await fetch(API_ENDPOINTS.getPortHealth(cmtk, selectedPort));
				...
				if (jsonData.data.tempWarning === true || jsonData.data.tempFailure === true) {
					setTempWarning("🌡️");
				} else {
					setTempWarning("");
				}

				if (jsonData.data.vibWarning === true || jsonData.data.vibFailure === true) {
					setVibWarning("📳");
				} else {
					setVibWarning("");
				}
			...
			<div className="d-flex flex-column w-100">
				...
				<div className="d-flex mx-5 justify-content-center flex-column">
					...
					<div className="d-flex w-100 mt-1 justify-content-evenly">
						<div className="">
							<label htmlFor="fieldSelect" className="form-label fw-bold">
						...
						<div className="">
							<label htmlFor="timeSelect" className="form-label fw-bold">
								Seleccione Rango de Tiempo:
							</label>
							<select id="timeSelect" className="form-select" value={selectedTime} onChange={(e) => setSelectedTime(e.target.value)}>
						...
						{/* Port Selection */}
						<div className="">
							<label htmlFor="portSelect" className="form-label fw-bold">
								Seleccione Port:
							</label>
							<select
								id="portSelect"
							...
							>
								{names.length > 0 ? (
									...
									<option disabled>No hay puertos disponibles</option>
								)}
							</select>
						</div>
						{/* Config section */}
					...
					<div className="d-flex w-100 justify-content-center align-items-center">
						<div className="mt-5" style={{ maxWidth: "700px", width: "100%" }}>
							{loading ? (
								<div>Loading...</div>
							) : error ? (
								<div className="text-danger">Error: {error}</div>
							) : (
								<ChartLine x={cmtkDataT} y={cmtkDataSelected} datasetLabel={selectedField} chartTitle={`Historial ${selectedField}`} cmtk={cmtk} port={selectedPort} showAxis={true} />
							)}
						</div>
					</div>
				</div>
